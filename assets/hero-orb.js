/* ============================================================================
   hero-orb.js — the amorphous plasma orb beside the headline
   ----------------------------------------------------------------------------
   A sphere whose surface is pushed around by 3D noise, wrapped in a glowing
   wireframe shell and a drift of orbiting sparks. The background stays fully
   transparent, so the page's own particle mesh keeps showing through it.
   ...
   How the morphing works — the part worth understanding:
     ...
     Neighbouring vertices read nearby points of that field and so get nearby
     values, which is why the surface bulges and dents smoothly instead of
     turning into static.

     The *shape-shifting* part is that the field's settings — how tall the bumps
     are, how fine they are, how much the field is twisted before it is read —
     are not constants. Every few seconds the CPU rolls a new set of targets and
     eases the live values toward them (see MorphState below). One moment the
     orb is a slow lava blob, a while later a spiky sea urchin, then a rippling
     drop. Nothing is keyframed and it never repeats exactly.

   Everything is additive-blended with depth writes off, which is what gives the
...
(function () {
  ...
  /* No `precision` line in these fragment shaders on purpose: three.js prepends
     the same qualifier to both stages, and declaring a different one by hand
     makes uniforms shared with the vertex shader fail to link. */
  var CORE_FRAG = [
    ...
    /* Fresnel: surfaces facing away from the viewer glow, which reads as a rim
       of light around the silhouette — the biggest single "this is plasma" cue. */
  ...
  /* Holds one live value and one target per shape parameter, plus the countdown
     to the next re-roll. Deliberately dumb: roll() picks new targets, step()
     eases the live values toward them a little every frame. */
  function MorphState() {
    this.live = {};
    this.target = {};
    this.timer = 0;
  ...
  function HeroOrb(THREE, host) {
    ...
    this.uniforms = {
      ...
      uPixelRatio: { value: 1 },
      uColorDeep:  { value: new THREE.Color("#1b2350") },
      uColorMid:   { value: new THREE.Color("#57e3d0") },
      uColorHot:   { value: new THREE.Color("#c58bff") },
      uOpacity:    { value: 1 },
    };
    ...
    this.coreMat = new THREE.ShaderMaterial({
      ...
      /* FrontSide, not DoubleSide: with additive blending the back half of the
         sphere would be added on top of the front half and double the
         brightness. */
    ...
    /* A coarser mesh for the wireframe: at detail 5 the lines merge into a
       solid haze. */
  ...
  HeroOrb.prototype.syncTheme = function () {
    ...
    /* Deep body in a dimmed primary, ridges in the second accent, rim in the
       third. Because `heat` is driven by displacement, the ridge colour only
       shows up on the parts of the surface currently bulging outward. */
    this.uniforms.uColorDeep.value.copy(cssColor(THREE, "--fx-c1", "#7c9cff")).multiplyScalar(additive ? 0.30 : 0.5);
    this.uniforms.uColorMid.value.copy(cssColor(THREE, "--fx-c2", "#57e3d0"));
    this.uniforms.uColorHot.value.copy(cssColor(THREE, "--fx-c3", "#c58bff"));

    /* Additive blending only works over a dark ground: on a light theme every
       overlapping layer climbs toward white and the orb turns into a pale
       smear. There it blends normally instead, with the alpha scaled up. */
    ...
    /* With normal blending the draw order suddenly matters: without a depth
       ...
       torn. Additive blending is order-independent, so a dark theme can keep
       depth writes off and stay glowy. */
    this.coreMat.depthWrite = !additive;
    this.uniforms.uOpacity.value = additive ? 1 : 1.7;
  };
  ...
  function init() {
    ...
    /* The column is display:none below the two-column breakpoint; building a
       WebGL context for a zero-sized element is pure waste. Watch for the
       window growing past that breakpoint so a resized narrow window still
       gets an orb. */
